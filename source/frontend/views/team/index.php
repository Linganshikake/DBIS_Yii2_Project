 ...
 * This is the frontend team index view (前端队伍列表视图)
 */
        ...
        <?php foreach ($teams as $team): ?>
            <div class="col-lg-4 col-md-6 col-sm-12" style="margin-bottom: 30px;">
                <a href="<?= Url::to(['team/view', 'id' => $team->id]) ?>" class="team-card-link" style="display: block; text-decoration: none;">
                    ...
                    <div class="team-card" style="height: 100%; padding: 0; overflow: hidden; position: relative; background: #1a1a1a; border: 1px solid #333; border-radius: 8px; transition: transform 0.2s;">
                        
                        <div style="height: 150px; background: #fff; display: flex; align-items: center; justify-content: center; border-bottom: 1px solid #d4af37; overflow: hidden; padding: 20px;">
                            <?php if ($team->logo): ?>
                                <img src="/uploads/teams/<?= $team->logo ?>" alt="<?= Html::encode($team->name) ?>" style="width: 100%; height: 100%; object-fit: contain;">
                            <?php else: ?>
                                <div style="text-align: center;">
                                    <span style="font-size: 40px; font-weight: 900; color: #333; letter-spacing: -2px;">M.L</span>
                                    <br>
                                    <span style="font-size: 12px; color: #999;">OFFICIAL TEAM</span>
                                </div>
                            <?php endif; ?>
                        </div>
